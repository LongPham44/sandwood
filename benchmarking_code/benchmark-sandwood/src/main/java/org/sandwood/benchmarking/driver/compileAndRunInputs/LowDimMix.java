...
public class LowDimMix {
    ...
    private static final Path OBSERVED_DATA_FILE = RESOURCES_DIR.resolve(
            Path.of("inputs", "org", "sandwood", "benchmarking", "observedData", "LowDimMix", "observed-data.json"));
    ...
    public static Map<TestType, TestData> getInputs() {
        ...
        Map<TestType, TestData> m = new HashMap<>();
        {
            ...
            t.outputNames = new String[] { "mu", "sigma", "theta" };
    ...
    private static InputStream openObservedData() throws IOException {
        ...
        InputStream in = LowDimMix.class.getClassLoader().getResourceAsStream(resourceName);
        if(in != null) {
            return in;
        }
