...
public class ARMA {
    ...
    private static final Path OBSERVED_DATA_FILE = RESOURCES_DIR.resolve(
            Path.of("inputs", "org", "sandwood", "benchmarking", "observedData", "ARMA", "observed-data.json"));
    ...
    public static Map<TestType, TestData> getInputs() {
        ...
        Map<TestType, TestData> m = new HashMap<>();
        {
            ...
            t.outputNames = new String[] { "mu", "phi", "theta", "sigma" };
    ...
    private static InputStream openObservedData() throws IOException {
        ...
        InputStream in = ARMA.class.getClassLoader().getResourceAsStream(resourceName);
        if(in != null) {
            return in;
        }
