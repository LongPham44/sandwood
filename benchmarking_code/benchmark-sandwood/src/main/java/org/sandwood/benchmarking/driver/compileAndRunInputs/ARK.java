...
public class ARK {
    ...
    private static final Path OBSERVED_DATA_FILE = RESOURCES_DIR.resolve(
            Path.of("inputs", "org", "sandwood", "benchmarking", "observedData", "ARK", "observed-data.json"));
    ...
    public static Map<TestType, TestData> getInputs() {
        ...
        Map<TestType, TestData> m = new HashMap<>();
        {
            ...
            t.inputs.put("K", requireInteger(observedData.K, "K"));
            t.inputs.put("yObserved", requireArray(observedData.y, "y"));
            t.args = new String[] { "K", "yObserved" };
            t.outputNames = new String[] { "alpha", "beta", "sigma" };
    ...
    private static InputStream openObservedData() throws IOException {
        ...
        InputStream in = ARK.class.getClassLoader().getResourceAsStream(resourceName);
        if(in != null) {
            return in;
        }
