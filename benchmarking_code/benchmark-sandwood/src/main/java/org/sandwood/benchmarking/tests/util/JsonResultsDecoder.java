...
public class JsonResultsDecoder {
    ...
    public static Map<String, Object> readData(String filename) throws SandwoodJsonException, IOException {
        ...
        while(!d.isObjectEnd()) {
            ...
            if(name.equals(JsonResultsEncoder.metadataName)) {
                readMetadata(d);
                d.leaveObject();
                continue;
            }
    ...
    private static void readMetadata(JsonDecoder d) throws SandwoodJsonException, IOException {
        while(!d.isObjectEnd()) {
            String name = d.getFieldName();
            switch(name) {
                case "runningTimeSeconds":
                    d.getDouble();
                    break;
                case "samplingConfiguration":
                    d.enterObject();
                    readSamplingConfiguration(d);
                    d.leaveObject();
                    break;
                default:
                    d.close();
                    throw new SandwoodTestException("Unknown metadata field");
            }
        }
    }

    private static void readSamplingConfiguration(JsonDecoder d) throws SandwoodJsonException, IOException {
        while(!d.isObjectEnd()) {
            String name = d.getFieldName();
            switch(name) {
                case "numChains":
                case "numWarmupsPerChain":
                case "numPosteriorSamplesPerChain":
                    d.getInt();
                    break;
                default:
                    d.close();
                    throw new SandwoodTestException("Unknown sampling configuration field");
            }
        }
    }
}
